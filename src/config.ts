...
import {
  ...
  type ConsultationNetworkMode,
  type ConsultationOwnerConfig,
...
function parseNetworkMode(value: unknown): ConsultationNetworkMode {
...
function parseOptionalUrl(
  value: unknown,
  fieldName: string,
  allowedProtocols: ReadonlyArray<"http:" | "https:" | "ws:" | "wss:">,
): string | undefined {
  ...
  if (!allowedProtocols.includes(url.protocol as "http:" | "https:" | "ws:" | "wss:")) {
...
function parseOwners(value: unknown): ConsultationOwnerConfig[] {
  if (!Array.isArray(value)) {
    return [];
  }

  const owners: ConsultationOwnerConfig[] = [];
  for (const entry of value) {
    ...
    const agentIds = Array.isArray(owner.agentIds)
      ? owner.agentIds.filter((item): item is string => typeof item === "string" && item.trim().length > 0)
      : [];
...
export function parseConsultationConfig(raw: unknown): ConsultationConfig {
