...
import { CONSULTATION_LIMIT_MESSAGE, CONSULTATION_PROTOCOL_VERSION, CONSULTATION_TRANSCRIPT_NOTICE, CONSULTATION_UNAVAILABLE_MESSAGE, } from "./consultation-types.js";
...
export class ConsultationStore {
    ...
    async createPolicy(ownerId, input) {
    ...
    async revokeGrant(ownerId, grantId, reason) {
        return this.withMutation(async () => {
    ...
    async startSession(rawToken, client) {
        return this.withMutation(async () => {
            const hashed = hashToken(rawToken);
            const grant = this.state.grants.find((entry) => entry.token_hash === hashed);
            if (!grant) {
                throw Object.assign(new Error(CONSULTATION_UNAVAILABLE_MESSAGE), { closeReason: "auth_failed" });
            }
    ...
    async reserveTurn(sessionId, messageId, prompt) {
        return this.withMutation(async () => {
            if (!messageId.trim()) {
            ...
            const existing = this.state.messages.find((entry) => entry.session_id === sessionId && entry.client_message_id === messageId && entry.direction === "requester_to_agent");
    ...
    async finalizeDenied(sessionId, messageId, matchedRuleIds, event) {
        return this.finalizeTurn(sessionId, messageId, "deny", matchedRuleIds, event);
    }
    async finalizeAllowed(sessionId, messageId, matchedRuleIds, event, adapterCallId) {
        return this.finalizeTurn(sessionId, messageId, "allow", matchedRuleIds, event, adapterCallId);
    }
    async finalizeError(sessionId, messageId, event) {
        return this.finalizeTurn(sessionId, messageId, "error", [], event);
    }
    async canDeliverToSession(sessionId) {
        return this.withMutation(async () => {
            const session = this.state.sessions.find((entry) => entry.id === sessionId);
            if (!session || session.status !== "open") {
    ...
    async finalizeTurn(sessionId, messageId, decision, matchedRuleIds, event, adapterCallId) {
        return this.withMutation(async () => {
            const message = this.state.messages.find((entry) => entry.session_id === sessionId && entry.client_message_id === messageId && entry.direction === "requester_to_agent");
            if (!message) {
                return {};
            }
    ...
    validateClientAgainstGrant(grant, client) {
        ...
        if (client.type === "agent") {
            if (!client.agent_id || !client.run_id || !client.purpose || !client.response_format) {
                throw Object.assign(new Error("Missing required agent client fields"), { closeReason: "protocol_error" });
            }
            if (client.response_format !== "text" && client.response_format !== "json_object") {
                throw Object.assign(new Error("Unsupported response_format"), { closeReason: "protocol_error" });
            }
        }
    ...
    determineGrantCloseReason(grant) {
        this.syncGrantStatus(grant);
        if (grant.status === "revoked") {
    ...
    closeSessionRecord(session, reason) {
    ...
    requirePolicy(policyId, ownerId) {
...
export function buildSessionStartedPayload(start) {
    return {
