import { CONSULTATION_GENERIC_DENY_MESSAGE, } from "./consultation-types.js";
const POLICY_COMPILED_VERSION = 1;
const DEFAULT_MAX_PROMPT_CHARS = 4_000;
function makeRuleId(prefix, index) {
...
function normalizeRules(rules, prefix) {
        ...
        case "general-safe-consult":
            return {
                ...
                rules: [
                    ...
                    ...(blockedKeywords.length > 0 ? [{ id: "deny-blocked-keywords", effect: "deny", type: "contains_any", values: blockedKeywords }] : []),
                ],
            };
        case "topic-limited":
...
export function compileConsultationPolicy(ownerId, policyId, input, nowIso) {
    if (!input.name?.trim()) {
        throw new Error("Policy name is required");
    }
    const template = templateRules(input);
    const rules = normalizeRules([...template.rules, ...(input.rules || [])], input.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") || "rule");
...
function matchesRule(rule, prompt, client) {
    switch (rule.type) {
        case "contains_any":
...
export function evaluateConsultationPolicy(policy, prompt, client) {
